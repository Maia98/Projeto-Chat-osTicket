...
var scp_prep = function() {
    ...
    $('.dialog').delegate('input#close-modal, a.close', 'click', function(e) {
        e.preventDefault();
...
$.dialog = function (url, codes, cb, options) {
    ...
    $('div.modal-content', $popup).empty();
    $('div.modal-content', $popup).load(url, function () {
        waitingDialog.hide();
        $('div.modal-content', $popup).slideDown({
            duration: 300,
            queue: false,
            complete: function() { if (options.onshow) options.onshow(); }
        });
        $(document).off('.dialog');
        $(document).on('submit.dialog', '#popup form', function(e) {
            e.preventDefault();
            var $form = $(this);
            waitingDialog.hide();
            $.ajax({
                ...
                success: function(resp, status, xhr) {
                    if (xhr && xhr.status && codes && $.inArray(xhr.status, codes) != -1) {
                        $.toggleOverlay(false);
                        $popup.hide();
                        $('div.modal-content', $popup).empty();
                        ...
                        $('div.modal-content', $popup).html(resp);
                        $popup.effect('shake');
      ...
      $('div.body', $popup).empty()
        ...
        .append($('<p class="full-width"></p>')
            .append($('<span class="buttons pull-left"></span>')
                .append($('<input class="btn btn-danger" type="button" class="close"/>')
                    .attr('value', __('Cancel'))
                    .click(function() { hide(); })
            )).append($('<span class="buttons pull-right"></span>')
                .append($('<input class="btn btn-primary" type="button"/>')
                    .attr('value', __('OK'))
...
$('#new-note').live('click', function() {
  var note = $(this).closest('.quicknote'),
    T = $('<textarea>'),
    button = $('<input type="button" class="btn btn-primary btn-notes">').val(__('Create'));
...
$("body").on("click", "#redactor-image-editter", function () {
   console.log("clicou");
});
$("#redactor_modal").css("background-color", "pink");