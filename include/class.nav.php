...
class StaffNav extends Staff{
    ...
    function getTabs(){
        ...
        if(!$this->tabs) {
            ...
            $this->tabs['relatorios'] = array('desc'=>__('Relatórios'), 'href'=>'relatorios.php', 'title'=>__('Relatórios'));
            $this->tabs['chamados'] = array('desc'=>__('Chamados'), 'href' => 'chamados.php', 'title' =>__('Chamados'));
    ...
    function getSubMenus(){ //Private.
        ...
        foreach($this->getTabs() as $k=>$tab){
            ...
            switch(strtolower($k)){
                case 'relatorios':
                //     $subnav[]=array('desc'=>__('Todos Tickets'), 'href'=>'relatorio.php?filter=ticket&value=all','iconclass'=>'logs', 'target' => '_blank');
                //     $subnav[]=array('desc'=>__('Tickets Abertos'), 'href'=>'relatorio.php?filter=ticket&value=aberto','iconclass'=>'logs', 'target' => '_blank');
                //     $subnav[]=array('desc'=>__('Tickets Fechados'), 'href'=>'relatorio.php?filter=ticket&value=fechado','iconclass'=>'logs', 'target' => '_blank');
                    $subnav[]=array('desc'=>__('Central de Relatórios'), 'href'=>'relatorios.php', 'title' => 'Central de Relatórios', 'iconclass'=>'logs');
                    break;
                case 'tickets':
        ...
        if(!$this->navs){
            ...
            if ($cfg->getClientRegistrationMode() != 'disabled'
                || !$cfg->isClientLoginRequired())
                    $navs['new']=array('desc'=>__('Entrar como Agente'),'href'=>'scp/login.php','title'=>'');

               
            $navs['new1']=array('desc'=>__('Entrar como Usuário'),'href'=>'tickets.php?a=open','title'=>'');
            ...
            if($user && $user->isValid()) {
                if(!$user->isGuest()) {
                    ...
                    $navs['new1']=array('desc'=>__('Sair'),
                        'href'=>sprintf('logout.php?auth=%d', $user->getLinkToken()),
                        'title'=>'');
                } else {
