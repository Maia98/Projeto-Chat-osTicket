...
class Pdf {
    ...
    {
        $this->mpdf  = new mPDF();
        $this->style    = "h1{margin:0;padding:0;color: red;}.pdfHeader{width:100%;border-bottom: solid 1px #cecece;padding-bottom: 20px;}.pdfHeader .logo{width: 50%;float: left;}.pdfHeader .title{width: 50%;font-size: 12pt;text-align: right;padding-top: 25px;}.pdfHeader .title span{display:inline-block;font-weight: bold;}table{width:100%;margin-top:20px;font-size:9pt;} table th{text-align:left;padding-bottom: 10px;}table tbody tr td{display: table; float:left;padding:5px;margin-right:10px;}table tbody tr:nth-child(odd){background-color:#eaeaea;}";
        $this->title    = "Exemplo PDF";
        $this->nameFile = "exemplo_pdf_".date("d_m_Y");
        $this->logo     =  ROOT_PATH . 'logo.php';

    }

    function setOrientation($orientation='P'){
    ...
    function createPdf(){
        ...
        $this->mpdf->WriteHTML("<style>".$this->style."</style>", 1);
        $this->mpdf->WriteHTML($html);
    }

    function openPdf(){
        $this->mpdf->Output("$this->nameFile", "I");
    }
    ...
    function setNameFile($nameFile){
        $this->nameFile = $nameFile."_".date("d_m_Y").".pdf";
    }
