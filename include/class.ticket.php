...
class Ticket {
        ...
        switch($status->getState()) {
            case 'closed':
                ...
                if ($dept->autoRespONNewTicket()) {
                    $subj= "Ticket Encerrado";

                    // Avaliação de atendimento
                    //$body= "O seu Ticket #" .$this->getNumber(). " foi encerrado em nosso sistema.  Caso queira avaliar esse atendimento <a href='https://docs.google.com/forms/d/e/1FAIpQLSedRjIO2LS5n2tFcdKktwIFTLjdrClA8RDhkOtQRhExxCzngg/viewform?entry.1302747347=" .$this->getNumber(). "'>Clique Aqui.</a>";
                    

                    $body= "O seu Ticket #" .$this->getNumber(). " foi encerrado em nosso sistema.";

                    $body .= '<div style="color:rgb(127,127,127)"> 
                                <em>Seus amigos da Equipe de Suporte</em> Prener,<br> <br>
                                <a href="http://' .$_SERVER["SERVER_NAME"]. '/" target="_blank">
                                <img src="http://' .$_SERVER["SERVER_NAME"]. '/logo.php" alt="Helpdesk Prener" width="170" height="55" class="CToWUd"></a>
                                <br>
                            </div>
                            <hr>
                            <div style="color:rgb(127,127,127);font-size:small;text-align:center">
                                <em>Esperamos que esta resposta tem suficientemente respondido suas perguntas.&nbsp;Se não, por favor, não enviar outro e-mail. Em vez disso, responda a este e-mail ou <a href="http://' . $_SERVER["SERVER_NAME"]. '/" style="color:rgb(84,141,212)" target="_blank">faça o login na sua conta</a> para ter acesso completo a todos os seus pedidos de suporte&nbsp;e respostas.</em>
                            </div>';
                    //$body= "This is a notification that Request#" .$this->getNumber(). " has been closed.  You can see request information here: http://" . $_SERVER["SERVER_NAME"] . "/view.php?e=" . $this->getEmail() . "&t=" . $this->getNumber() . ".";
    ...
    function onNewTicket($message, $autorespond=true, $alertstaff=true) {
        ...
        if ($alertstaff
            ...
            if($cfg->alertDeptManagerONNewTicket() && $dept && ($manager=$dept->getSubManager()))
                $recipients[]= $manager;
   ...
   function onOverdue($whine=true, $comments="") {
        ...
        if(($tpl = $dept->getTemplate())
            ...
            if($cfg->alertDeptManagerONOverdueTicket() && $dept && ($manager=$dept->getManager()) && ($manager=$dept->getSubManager()))
                $recipients[]= $manager;
    ...
    function postMessage($vars, $origin='', $alerts=true) {
        ...
        if($cfg->alertONNewMessage()
            ...
            //Dept manager
            if($cfg->alertDeptManagerONNewMessage() && $dept && ($manager=$dept->getManager() && $manager=$dept->getSubManager()))
                $recipients[]=$manager;
    ...
    function update($vars, &$errors) {
        ...
        unset($vars["source"], $vars["topicId"]);
...
//            $source = 'Email';
        ...
        //Saving SLA.
        if (isset($vars['slaId']))
            $vars['slaId'] = $vars['slaId'];
        else{
            $vars['slaId'] = $cfg->getDefaultSLAId();
        }
...
//        $source = $source ?: 'Web';
        ...
        $sla_id_esc = $vars['slaId']; // Get id sla
        $resultSla  = db_fetch_row(db_query("SELECT grace_period FROM ostau_sla WHERE id = $sla_id_esc")); // Search hours sla
        $resultSla  = $resultSla[0]; // Get result sql
        $addDate = "+$resultSla hours"; // Variable save result
        $newDate = date("Y-m-d H:m:s", strtotime($addDate)); // Add result sql in date current
   ...
   function filtros_padrao($query, $data_ini, $data_fim, $departamento_selected, $assunto_selected, $status){
       
        if($data_ini && $data_fim){
            $query .= " AND ti.created BETWEEN '".$data_ini." 00:00:00' AND '".$data_fim." 23:59:59'";
        }
        
        if ($departamento_selected) {
            $query .= " AND de.dept_id = '".$departamento_selected."'";
        }
        if($assunto_selected){
            $query .= " AND cdata.subject = '".$assunto_selected."'";
        }
        if($status == 'aberto'){
            $query .= "AND ti.status_id = 1 AND ti.closed is null ";
        }//
        if ($status == 'fechado'){
            $query .= "AND ti.status_id = 3 AND ti.closed is not null ";
        }//
