...
<div class="modal-body">
  <?php echo __(
  'As seqüências são usadas para gerar números seqüenciais. Várias seqüências podem ser
   usadas para gerar seqüências para diferentes fins.'); ?>
  <br/>
  <br/>
  <form method="post" action="<?php echo $info['action']; ?>">
    <div id="sequences" style="max-height: 250px; overflow-y: scroll; ">
    <?php
    $current_list = array();
    ...
    <div class="row-item hidden" id="template">
        ...
        <div class="manage-buttons pull-right new-manage">
            <span class="faded">next</span>
    ...
    <button class="btn btn-default" onclick="javascript:
      ...
      var clone = $('.row-item.hidden#template').clone()
        .appendTo($('#sequences'))
        .removeClass('hidden')
    ...
    <hr>
    <div>
      <button type="submit" class="btn btn-success pull-right" onclick="javascript:
    $('#sequences .save a').each(function() { $(this).trigger('click'); }); $('#popup').modal('toggle');location.reload();
...
<script>
    ...
    if($(window).width() <= 450) {
        $(".row-item .manage a").click(function () {

            var className = $(this).parent().attr('class');

            if (className === 'manage') {
                $(this).parent().parent().parent().css({'padding-bottom': '25%'});
            } else {
