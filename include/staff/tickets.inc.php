...
$qwhere =' WHERE ( '
        ...
        .' AND status.state="'.$_REQUEST['status'].'") ';
...
<div>
        <div class="pull-left flush-left">
            ...
            <div class="pull-right flush-right change-status">
                <?php
    ...
    <div id='basic_search'>
        <form action="tickets.php" method="get">
            ...
            <div class="pull-right flush-right" style="width: 100%; margin-top: 10px;">
                <input type="text" class="form-control" id="basic-ticket-search" name="query" value="<?php echo Format::htmlchars($_REQUEST['query'],true); ?>" autocomplete="off" autocorrect="off" autocapitalize="off">
                <input type="submit" name="basic_search" class="form-control btn-primary pull-right" value="<?php echo __('Search'); ?>">
            </div>
        </form>
    </div>
...
<?php csrf_token(); ?>
 ...
 <table class="table table-striped table-bordered ticket-check" border="0" cellspacing="1" cellpadding="2" width="940">
    <thead>
        <tr>
     ...
     <tbody>
        ...
        if($res && ($num=count($results))):
            ...
            foreach ($results as $row) {
                ...
                if($showassigned) {
                    if($row['staff_id'])
                        $lc=sprintf('%s',Format::truncate($row['staff'],40));
                    elseif($row['team_id'])
                        $lc=sprintf('%s',Format::truncate($row['team'],40));
                    else
                        $lc=' ';
                }else{
    ...
    if ($num>0) { //if we actually had any tickets returned.
        ...
        echo sprintf('<a class="btn btn-success no-pjax" href="?%s">%s</a>',
...
<div class="dialog" style="display:none;" id="advanced-search">
    ...
    <form action="tickets.php" method="post" id="search" name="search">
        ...
        <fieldset class="query">
            <input type="input" id="query" class="form-control" name="query" size="20" placeholder="<?php echo __('Keywords') . ' &mdash; ' . __('Optional'); ?>">
        </fieldset>
        ...
        <fieldset class="date_range">
            ...
            <input class="dp" class="form-control" type="input" size="20" name="startDate">
            <span class="between"><?php echo __('TO');?></span>
            <input class="dp" class="form-control" type="input" size="20" name="endDate">
        </fieldset>
        <?php
        ...
        <p>
            <span class="buttons pull-right">
                <input type="submit" class="form-control" value="<?php echo __('Search');?>">
            </span>
            <span class="buttons pull-left">
