...
class Dept {
    ...
    function getSubManagerId() {
        return $this->ht['sub_manager_id'];
    }
    ...
    function getSubManager() {

        if(!$this->subManager && $this->getSubManagerId())
            $this->subManager=Staff::lookup($this->getSubManagerId());
    ...
    function isSubManager($staff) {

        if(is_object($staff)) $staff=$staff->getId();

        return ($this->getSubManagerId() && $this->getSubManagerId()==$staff);
    }
    ...
    function save($id, $vars, &$errors) {
        ...
        $sql='SET updated=NOW() '
            ...
            .' ,sub_manager_id='.db_input($vars['sub_manager_id']?$vars['sub_manager_id']:0)
