...
class RadioField extends FormField {
    ...
    function getChoices($verbose=false) {
        ...
        return Staff::getAvailableStaffMembers();
    }
...
class RadioWidget extends Widget {
    ...
    function render($options=array()) {
        ...
        // selected)
        $choices = Staff::getAvailableStaffMembers();
    ...
    function emitChoices($choices, $values=array(), $have_def=false, $def_key=null) {
        ...
        ?><select name="<?php echo $this->name; ?>[]">
            <option value="0" selected="selected">&mdash; <?php echo __('Selecione Agente');?> &mdash;</option><?php
        foreach ($choices as $key => $name) {
            if (!$have_def && $key == $def_key)
                continue; ?>
            <option type="radio" name="<?php echo $this->name; ?>[]" value="<?php echo $key; ?>"<?php
