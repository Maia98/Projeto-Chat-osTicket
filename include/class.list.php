...
class DynamicListItem extends VerySimpleModel implements CustomListItem {
    ...
    function getDept() {
        return $this->get('dept_id');
    }

    function getSla() {
        return $this->get('sla_id');
    }
    ...
    function update($vars, &$errors=array()) {
        ...
        $config = $this->getConfigurationForm();
        $config = $config->_source;
        $idLine = $this->ht['id'];
        $dept   = $config["dept-$idLine"];
        $sla    = $config["sla-$idLine"];
        ...
        foreach (array(
            ...
            if($k == 'dept'){
                $this->set("dept_id", $dept);
            }else if($k == 'sla'){
                $this->set("sla_id", $sla);
            }else if(isset($vars[$k])){
                $this->set($v, $vars[$k]);
            }
        }
        return $this->save();
