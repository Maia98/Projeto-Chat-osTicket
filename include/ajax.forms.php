...
class DynamicFormsAjaxAPI extends AjaxController {
    ...
    function getSubjects() {
        try{
            ...
            $id       = $_POST['id'];
            $sql      = "SELECT * FROM ostau_list_items WHERE list_id = 4 AND dept_id = ".$id."";
            ...
            return json_encode(["success" => true, "subjects" => $subjects]);
    ...
    function getSla() {
        try{
            $id     = $_POST['id'];
            $sql    = "SELECT ostau_sla.id AS id, ostau_sla.name AS name, ostau_sla.grace_period AS period FROM ostau_list_items AS li INNER JOIN ostau_sla ON ostau_sla.id = li.sla_id  WHERE li.list_id = 4 AND li.id = $id";
            $result = db_query($sql);
            $sla    = array();
            while($row = db_fetch_array($result)){
                array_push($sla, $row);
            }
            return json_encode(["success" => true, "sla" => $sla]);
        }catch (Exception $e){
            return json_encode(["success" => false]);
        }
    }
